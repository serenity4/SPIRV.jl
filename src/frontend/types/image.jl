...
(img::SampledImage)(coords::Vec{<:Any,<:IEEEFloat}, lod::Integer) = ImageSampleExplicitLod(img, coords, lod)
(img::SampledImage)(coords::V, dx::V, dy::V) where {V<:Vec{<:Any,<:IEEEFloat}} = ImageSampleExplicitLod(img, coords, dx, dy)

@noinline ImageSampleImplicitLod(img::SampledImage, coord) = zero(sampled_type(typeof(img)))
@noinline ImageSampleExplicitLod(img::SampledImage, coord, lod::Integer) = zero(sampled_type(typeof(img)))
@noinline ImageSampleExplicitLod(img::SampledImage, coord, dx, dy) = zero(sampled_type(typeof(img)))
